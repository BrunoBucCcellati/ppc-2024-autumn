  ...
  do {
    ...
    while (i != local_output_x_vector_part_.size()) {
      ...
      if (world.rank() == 0) {
        ...
        double new_value = (local_input_right_vector_part_[i] - sum) * (1.0 / local_input_matrix_part_[i * (n + 1) + (world.rank() - 1) * (number_of_local_matrix_rows)]);
        local_output_x_vector_part_[i] = new_value;
