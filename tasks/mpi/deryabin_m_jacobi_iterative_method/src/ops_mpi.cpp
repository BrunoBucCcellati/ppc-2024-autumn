...
bool deryabin_m_jacobi_iterative_method_mpi::JacobiIterativeMPITaskParallel::pre_processing() {
  ...
  if (world.rank() == 0) {
    ...
    for (int proc = 1; proc < world.size(); proc++) {
      world.send(proc, 0, 
                 input_matrix_.data() + (proc - 1) * number_of_local_matrix_rows * n,
  ...
  if (world.rank() == 0) {
    local_input_matrix_part_ = std::vector<double>(
        input_matrix_.end() - 
            (number_of_local_matrix_rows + ostatochnoe_chislo_strock) * n, 
        input_matrix_.end());
...
bool deryabin_m_jacobi_iterative_method_mpi::JacobiIterativeMPITaskParallel::validation() {
  ...
  if (world.rank() == 0) {
    ...
    for (int proc = 1; proc < world.size(); proc++) {
      world.send(proc, 0, matrix_.data() + (proc - 1) * number_of_local_matrix_rows * n, 
                 number_of_local_matrix_rows * n);
    }
  }
  boost::mpi::broadcast(world, number_of_local_matrix_rows, 0);
  ...
  std::vector<double> loc_matrix_part_ = 
      std::vector<double>(number_of_local_matrix_rows * n);
  if (world.rank() == 0) {
    loc_matrix_part_ = std::vector<double>(
        matrix_.end() - (number_of_local_matrix_rows + ostatochnoe_chislo_strock) * n, 
        matrix_.end());
  } else {
...
bool deryabin_m_jacobi_iterative_method_mpi::JacobiIterativeMPITaskParallel::run() {
  internal_order_test();
  std::vector<double> vec(world.size(), 1);
  std::vector<double> v(world.size(), 0);
  std::vector<int> sendcounts(world.size() - 1, 1);
